...
#' @export
#'
runStepwiseRegK <- function(ggc, filt.data) {
...
###########################################
    # Find elbow point
    elbow_id <- findElbowK(y = log(scree_values)[1:100], ylab = "Log Variance Explained", plot = T)
...
############################################
    ...
    # Select potential candidates for next neighbour, changed to mclappy for faster computing
    candidateGGCList <-
